      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-[400px] bg-zinc-50 dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col`}>
        ...
        <div className="flex items-center justify-between p-5 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900">
          <div className="flex items-center space-x-2">
            <div className="p-2 bg-zinc-200 dark:bg-zinc-700/30 text-zinc-900 dark:text-zinc-100 rounded-lg">
              <Store size={20} />
            </div>
            <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">My Jalandhar Shop</h2>
          </div>
          <div className="flex items-center space-x-3">
            <span className="bg-zinc-200 text-zinc-700 dark:bg-zinc-700/50 dark:text-zinc-300 text-xs font-bold px-2 py-1 rounded-full select-none">
              {inventory.length} Items
            </span>
            <button 
              onClick={onClose} 
              aria-label="Close Inventory Drawer"
              className="p-1.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors active:scale-[0.97] custom-focus-ring"
            >
              <X size={20} />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar space-y-3">
          {inventory.length === 0 ? (
            <div className="text-center text-zinc-500 py-10 text-sm flex flex-col items-center">
            ...
            inventory.map((item) => (
              <div key={item.id} className="p-4 bg-white dark:bg-zinc-800 rounded-xl border border-zinc-100 dark:border-zinc-700/50 hover:border-zinc-300 dark:hover:border-zinc-700/50 transition-colors shadow-sm">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 leading-tight">{item.name}</h3>
                    <p className="text-xs text-zinc-500 mt-1">{item.category}</p>
                  </div>
                  <button 
                    onClick={() => removeItem(item.id)} 
                    aria-label="Remove item"
                    className="text-zinc-400 hover:text-red-500 transition-colors p-1 hover:bg-red-50 dark:hover:bg-red-900/20 rounded active:scale-[0.97] custom-focus-ring"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
                
                <div className="flex space-x-2 mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-700/50 select-none">
                  <button 
                    onClick={() => updateStatus(item.id, 'Stocked')}
                    className={`flex-1 flex items-center justify-center py-1.5 rounded-lg text-xs font-medium transition-colors active:scale-[0.97] custom-focus-ring ${item.status === 'Stocked' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400 ring-1 ring-green-500/30' : 'bg-zinc-50 dark:bg-zinc-700/50 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
                  >
                    <CheckCircle2 size={14} className="mr-1.5" /> In Store
                  </button>
                  <button 
                    onClick={() => updateStatus(item.id, 'Ordered')}
                    className={`flex-1 flex items-center justify-center py-1.5 rounded-lg text-xs font-medium transition-colors active:scale-[0.97] custom-focus-ring ${item.status === 'Ordered' ? 'bg-zinc-200 text-zinc-900 dark:bg-zinc-700/40 dark:text-zinc-100 ring-1 ring-zinc-300/30' : 'bg-zinc-50 dark:bg-zinc-700/50 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
                  >
                    <Clock size={14} className="mr-1.5" /> Ordered
                  </button>
                  <button 
                    onClick={() => updateStatus(item.id, 'Out of Stock')}
                    className={`flex-1 flex items-center justify-center py-1.5 rounded-lg text-xs font-medium transition-colors active:scale-[0.97] custom-focus-ring ${item.status === 'Out of Stock' ? 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400 ring-1 ring-red-500/30' : 'bg-zinc-50 dark:bg-zinc-700/50 text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-700'}`}
                  >
